...
#     result = ''
#     for key, value in sorted(all_item.items()):
#         if file1.get(key, None) == file2.get(key):
#             result += f'    {key}: {str(value).lower()}\n'
#         elif file1.get(key, None) is None:
#             result += f'  + {key}: {str(value).lower()}\n'
...
def main():
    ...
    data1 = parse_file(file1, format)
    data2 = parse_file(file2, format)
